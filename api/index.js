...
app.post('/api/inventory/initialize', async (req, res) => {
  try {
    ...
    if (existingCount > 0) {
      console.log(`📊 Inventory already has ${existingCount} items, skipping initialization`);
      return res.json({
    ...
    const sampleItems = [
      ...
      },
      {
        ...
        price: 12.99,
        quantity: 100,
        description: '1 meter USB-C to USB-C charging cable',
        sku: 'UC-003',
        image: '',
        createdAt: new Date(),
        updatedAt: new Date()
      },
      {
        ...
        price: 45.50,
        quantity: 20,
        description: 'Elegant ceramic decorative vase',
        sku: 'DV-004',
        image: '',
        createdAt: new Date(),
        updatedAt: new Date()
    ...
    console.log('💾 Saving sample inventory items...');
    await Inventory.insertMany(sampleItems);
    console.log(`✅ Sample inventory initialized with ${sampleItems.length} items`);

    res.json({
      message: 'Sample inventory initialized successfully',
